...
local function fallback_lno_chunks(win, lnum, width)
  ...
  local lno_str = string.format('%' .. lno_width .. 'd', display_lnum)
...
local function build_prefix(win, lnum, opts)
  ...
  if has_col and statuscol and statuscol ~= '' then
    ...
    local status_chunks = eval_statusline_chunks(statuscol, win, lnum)
    if status_chunks == nil then
      chunks = fallback_lno_chunks(win, lnum, width)
    else
      chunks = status_chunks
    end
  else
    chunks = eval_statusline_chunks('%C', win, lnum) or {}

    local body_chunks --- @type Gitsigns.VirtTextChunk[]
    if vim.wo[win].number or vim.wo[win].relativenumber then
      local fallback_width = math.max(width - chunks_width(chunks), 0)
      body_chunks = fallback_lno_chunks(win, lnum, fallback_width)
    end

    if not body_chunks or #body_chunks == 0 then
      local fallback_width = math.max(width - chunks_width(chunks), 0)
      body_chunks = fallback_lno_chunks(win, lnum, fallback_width)
    end
