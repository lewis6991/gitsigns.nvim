...
local function ensure_cached_source_buf(source_cache, bufnr, lines, cache_key)
  ...
  local version = lines
  local source_buf = source_cache[cache_key]
  ...
  if source_buf.version ~= version then
    api.nvim_buf_set_lines(source_buf.bufnr, 0, -1, false, lines)
    source_buf.version = version
  end
  ...
  vim.bo[source_buf.bufnr].bufhidden = 'hide'
  sync_source_buf_options(source_buf.bufnr, bufnr)
...
--- Prepare source buffers as needed and return popup lines for a hunk.
--- May allocate scratch buffers and wait for source rendering before capture.
--- @param bufnr integer
--- @param hunk Gitsigns.Hunk.Hunk
...
--- @return Gitsigns.LineSpec[]
function M.prepare_linespec_for_hunk(bufnr, hunk, removed_source, added_source, added_node)
